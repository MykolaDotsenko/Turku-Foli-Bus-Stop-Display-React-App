...
test("Turku demo is mobile-safe, accessible and hands off to real Kauppatori data", async ({
  page,
}, testInfo) => {
  test.skip(
    !["webkit-mobile", "chromium-mobile"].includes(testInfo.project.name)
  ...
  await page.getByRole("button", { name: "Try Turku demo" }).click();

  const dialog = page.getByRole("dialog", { name: "Know what leaves next" });
  await expect(dialog).toBeVisible();

  const fitsViewport = await dialog.evaluate((node) => {
    const rect = node.getBoundingClientRect();
    return {
      horizontal: rect.left >= 0 && rect.right <= globalThis.innerWidth,
      vertical: rect.top >= 0 && rect.bottom <= globalThis.innerHeight,
      noHorizontalOverflow: node.scrollWidth <= node.clientWidth + 1,
    };
  });

  expect(fitsViewport.horizontal).toBe(true);
  expect(fitsViewport.vertical).toBe(true);
  expect(fitsViewport.noHorizontalOverflow).toBe(true);
  ...
  await page.getByRole("button", { name: "Open Kauppatori" }).click();

  await expect(page).toHaveURL(/stop=164/);
  await expect(page.getByRole("heading", { name: "Kauppatori" })).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Try Turku demo" })
  ).toHaveCount(0);
});
